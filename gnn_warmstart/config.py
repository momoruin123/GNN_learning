...
# ========== GNN 模型参数 ==========
HIDDEN_DIM = 64                     # 隐藏层维度
NUM_CONV_LAYERS = 3                 # 消息传递层数（bipartite conv rounds）
DROPOUT = 0.1                       # Dropout 概率

# ========== 训练设施 ==========
BATCH_SIZE = 1                      # 批量大小（图数量，含分图结构需逐图处理）
GRAD_ACCUM_STEPS = 8                # 梯度累积步数（等效 batch_size = BATCH_SIZE * GRAD_ACCUM_STEPS）
NUM_WORKERS = 0                     # DataLoader 工作进程数（Windows 建议 0）
DEVICE = "cpu"                      # 训练设备 ("cpu" 或 "cuda")

# ========== 热启动参数 ==========
PREDICTION_THRESHOLD = 0.5          # 分类阈值（> threshold 则预测为 1）
